...
class XMLConfigure
{
	...
	static const char *XML_ATTRIBUTE_SIDE;
	static const char *XML_ATTRIBUTE_LEFT;
	static const char *XML_ATTRIBUTE_RIGHT;
	static const char *XML_ATTRIBUTE_BOTH;

	static const char *XML_ATTRIBUTE_MODEL;

	static const char *XML_ATTRIBUTE_GENDER;
	static const char *XML_ATTRIBUTE_MALE;
	static const char *XML_ATTRIBUTE_FEMALE;

	static const char *XML_ATTRIBUTE_RACE;
	static const char *XML_ATTRIBUTE_BLACK;
	static const char *XML_ATTRIBUTE_WHITE;

	static const char *XML_ATTRIBUTE_SCALE_X;
	static const char *XML_ATTRIBUTE_SCALE_Y;
	static const char *XML_ATTRIBUTE_SCALE_Z;

	static const char *XML_ATTRIBUTE_ROTATE_X;
	static const char *XML_ATTRIBUTE_ROTATE_Y;
	static const char *XML_ATTRIBUTE_ROTATE_Z;

	static const char *XML_ATTRIBUTE_POSITION_X;
	static const char *XML_ATTRIBUTE_POSITION_Y;
	static const char *XML_ATTRIBUTE_POSITION_Z;

	static const char *XML_ATTRIBUTE_RED;
	static const char *XML_ATTRIBUTE_GREEN;
	static const char *XML_ATTRIBUTE_BLUE;

	static const char *XML_ATTRIBUTE_CUTSCENE_START;
	static const char *XML_ATTRIBUTE_CUTSCENE_END;

	static const char *XML_ATTRIBUTE_ANIMATION_START;
	static const char *XML_ATTRIBUTE_ANIMATION_END;
	static const char *XML_ATTRIBUTE_ANIMATION_TIME;
	static const char *XML_ATTRIBUTE_ANIMATION_TYPE;
	static const char *XML_ATTRIBUTE_ANIMATION_STATIC;
	static const char *XML_ATTRIBUTE_ANIMATION_LOOP;

	static const char *XML_TAG_ROOT;

	static const char *XML_TAG_CUTSCENE;

	static const char *XML_TAG_CHARACTER;

	static const char *XML_TAG_HAND;

	static const char *XML_TAG_ANIMATION;

	static const char *HAND_MODEL_ORIGINAL;


	static bool AttributeIsId(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_ID);
	}

	static bool AttributeIsName(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_NAME);
	}

	static bool AttributeIsModel(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_MODEL);
	}

	static bool AttributeIsRace(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_RACE);
	}

	static bool AttributeIsGender(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_GENDER);
	}

	static bool AttributeIsSide(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_SIDE);
	}

	static bool AttributeIsScaleX(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_SCALE_X);
	}

	static bool AttributeIsScaleY(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_SCALE_Y);
	}

	static bool AttributeIsScaleZ(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_SCALE_Z);
	}

	static bool AttributeIsRotateX(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_ROTATE_X);
	}

	static bool AttributeIsRotateY(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_ROTATE_Y);
	}

	static bool AttributeIsRotateZ(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_ROTATE_Z);
	}

	static bool AttributeIsPositionX(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_POSITION_X);
	}

	static bool AttributeIsPositionY(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_POSITION_Y);
	}

	static bool AttributeIsPositionZ(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_POSITION_Z);
	}

	static bool AttributeIsRed(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_RED);
	}

	static bool AttributeIsGreen(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_GREEN);
	}

	static bool AttributeIsBlue(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_BLUE);
	}

	static bool AttributeIsCutsceneStart(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_CUTSCENE_START);
	}

	static bool AttributeIsCutsceneEnd(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_CUTSCENE_END);
	}

	static bool AttributeIsAnimationTime(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_ANIMATION_TIME);
	}

	static bool AttributeIsAnimationType(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_ANIMATION_TYPE);
	}

	static bool AttributeIsAnimationStart(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_ANIMATION_START);
	}

	static bool AttributeIsAnimationEnd(XmlLibrary::Attribute * const &attribute)
	{
		return !strcasecmp(attribute->Name().c_str(), XML_ATTRIBUTE_ANIMATION_END);
	}
...
public:
	static const char *HAND_MODEL_STICK;

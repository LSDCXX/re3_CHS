...
void XMLConfigure::Process()
{
	...
	{
		if ( !strcasecmp(m_curElemet->GetName().c_str(), XML_TAG_ROOT) )
		{
			ProcessChildren();
		}
		else if ( !strcasecmp(m_curElemet->GetName().c_str(), XML_TAG_CUTSCENE) )
		{
			Cutscene();
		}
		else if ( !strcasecmp(m_curElemet->GetName().c_str(), XML_TAG_CHARACTER) )
		{
			Character();
		}
		else if ( !strcasecmp(m_curElemet->GetName().c_str(), XML_TAG_HAND) )
		{
			Hand();
		}
		else if ( !strcasecmp(m_curElemet->GetName().c_str(), XML_TAG_ANIMATION) )
		{
			Animation();
		}
	}
...
void XMLConfigure::HandSide(const XmlLibrary::AttributeVector *attributes)
{
	if ( attributes )
	{
		...
		if ( it != attributes->end() )
		{
			if ( !strcasecmp((*it)->Value().c_str(), XML_ATTRIBUTE_RIGHT) )
				m_HandSide = CCutsceneHand::HANDEDNESS_RIGHT;
			else if ( !strcasecmp((*it)->Value().c_str(), XML_ATTRIBUTE_LEFT) )
				m_HandSide = CCutsceneHand::HANDEDNESS_LEFT;
			else if ( !strcasecmp((*it)->Value().c_str(), XML_ATTRIBUTE_BOTH) )
				m_HandSide = CCutsceneHand::HANDEDNESS_BOTH;
			else
...
void XMLConfigure::HandModel(const XmlLibrary::AttributeVector *attributes)
{
	if ( attributes )
	{
		...
		if ( it != attributes->end() )
		{
			...
			if ( !strcasecmp((*it)->Value().c_str(), HAND_MODEL_ORIGINAL) )
				modelname = "";
...
void XMLConfigure::HandGender(const XmlLibrary::AttributeVector *attributes)
{
	if ( attributes )
	{
		...
		if ( it != attributes->end() )
		{
			...
			if ( !strcasecmp((*it)->Value().c_str(), XML_ATTRIBUTE_MALE) )
				gender = CCutsceneHand::GENDER_MALE;
			else if ( !strcasecmp((*it)->Value().c_str(), XML_ATTRIBUTE_FEMALE) )
...
void XMLConfigure::HandRace(const XmlLibrary::AttributeVector *attributes)
{
	if ( attributes )
	{
		...
		if ( it != attributes->end() )
		{
			...
			if ( !strcasecmp((*it)->Value().c_str(), XML_ATTRIBUTE_BLACK) )
				race = CCutsceneHand::RACE_BLACK;
			else if ( !strcasecmp((*it)->Value().c_str(), XML_ATTRIBUTE_WHITE) )
...
void XMLConfigure::AnimationType(const XmlLibrary::AttributeVector *attributes)
{
	if ( attributes )
	{
		...
		if ( it != attributes->end() )
		{
			...
			if ( !strcasecmp((*it)->Value().c_str(), XML_ATTRIBUTE_ANIMATION_STATIC) )
				bIsStatic = true;
			else if ( !strcasecmp((*it)->Value().c_str(), XML_ATTRIBUTE_ANIMATION_LOOP) )
				bIsStatic = false;
